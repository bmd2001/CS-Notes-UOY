...
class Node(Connect4State):
    
    def __init__(self):
        self.children = []
        self.playerJustMoved = Connect4State.playerJustMoved
        self.gameState = Connect4State.GameState.Clone()
        self.minmax = 0
    ...
    def GetChildren(self):
        return [Node(Connect4State.Clone().DoMove(col)) for col in Connect4State.getMoves()]

def minMaxSearch(root):
    root.children = root.GetChildren()

    
env = Connect4State(width=2, height=3, connect = 2)
root = Node(env)
minMaxSearch(Node(env))
PlayGame(env)
