...
def BuildGameSubTree(node):
    ...
    for child in node.children:
        BuildGameSubTree(child)
        child.UpdateMinMax()

def heuristics(leaf):
    currentPlayer = 3 - leaf.playerJustMoved
    currentMax = 0
    for col in leaf.state.board:
        

    
def PlayGameMinMax(root, env):
    initialState = env
    root = BuildGameTree(root)
