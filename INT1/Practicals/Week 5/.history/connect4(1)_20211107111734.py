...
def PlayGameMinMax(root, env):
    ...
    while not state.IsGameOver():
        ...
        if state.playerJustMoved == 1:
            ...
            indeces = np.argwhere(np.array(min_value)==np.min(min_value)).flatten().tolist()
            indx = random.choice(indeces)
        else:
            max_value = max(minmax_values)            
            indeces = np.argwhere(np.array(minmax_values)==np.max(minmax_values)).flatten().tolist()
            indx = random.choice(indeces)
