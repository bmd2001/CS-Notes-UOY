...
def PlayGameMinMaxHeuristic(root, env):
    state = env
    current_node = BuildGameTreeH(root)
    while not state.IsGameOver():
        print(str(state))
        minmax_values = [child.minmax for child in current_node.children]
        if state.playerJustMoved == 1:
...
def BuildGameTreeH(root):
    root = BuildGameSubTreeH(root)
    return root

def BuildGameSubTreeH(node):
    if node.state.IsGameOver():
        node.UpdateMinMax(heuristic = True)
        return(node)
    children = node.GetChildren()
    for child in node.children:
        child = BuildGameSubTree(child)
        child.UpdateMinMax(heuristic = True)
    return(node)
...
PlayGameMinMaxHeuristic(Node(state = env), env)
