...
def PlayGameMinMaxHeuristic(root, env, lookahead = 4):
    state = env
    current_node = BuildGameTreeH(root, lookahead)
    while not state.IsGameOver():
        print(str(state))
        minmax_values = [child.minmax for child in current_node.children]
        if state.playerJustMoved == 1:
        ...
        state.DoMove(move)
        current_node = BuildGameTreeH(root.children[indx].state, depth=lookahead)
    PrintGameResults(state)

def BuildGameTreeH(root, depth = 5):
...
env = Connect4State(width=4, height=4, connect= 3)
PlayGameMinMaxHeuristic(Node(state = env), env, lookahead=4)
