...
def PlayGameMinMax(root, env):
    ...
    while not state.isGameOver():
        print(str(state))
        minmax_values = [minmax for minmax in current_node.children]
        if state.playerJustMoved == 1:
            min_value = min(minmax_values)
            indx = np.argwhere(np.array(min_value)==np.min(min_value)).flatten().tolist()
            indx = random.choice(indx)
        else:
            max_value = max(minmax_values)            
            indx = np.argwhere(np.array(max_value)==np.min(max_value)).flatten().tolist()
            indx = random.choice(indx)
        current_node, state = current_node.children[indx], current_node.children[indx]


    
    
env = Connect4State(width=4, height=3)
PlayGameMinMax(Node(state = env), env)
